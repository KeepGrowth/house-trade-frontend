    ...
    >
      ...
      <div v-show="currentStep === 3" class="preview-box">
        <el-descriptions title="房源信息预览" border :column="2">
          ...
          <el-descriptions-item label="户型">{{ formData.house_type }}</el-descriptions-item>
...
const nextStep = async () => {
  if (currentStep.value === 0) {
    // 校验第一步基础信息
    const valid = await formRef.value.validateField(['title', 'price', 'area', 'house_type', 'district', 'community']).catch(() => false);
    if (!valid) return;
  }
